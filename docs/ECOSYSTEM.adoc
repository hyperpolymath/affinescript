...
*CLOSED 2026-07-25* |Substrate (INT-01..04 + CORE-01) closed 2026-05-31;
remaining original #175 children INT-07 (#182) and INT-08 (#183) closed
2026-07-25. Residual WASI follow-ups #485 (S5 filesystem) and #487
(socket recv/send/accept) have preview1 lowerings + gated smokes in
tree; #486 flipped the wrap default to `--command` without the one-way
compiler re-target. Satellite remainder is #56 (56-A Dom/Console + Pixi
gap-fill + Window/Document/utility PR 4 landed; record-literal attrs
remain). Stage E is now the frontier.
...
ABI matured to "shared with Ephapax". |*ACTIVE — current frontier* |D's
substrate (INT-01..04, CORE-01) is closed. Ends at a stable, multi-producer
...
repository (`hyperpolymath/typed-wasm`), its own Idris2 proofs, its own
 surface, its own Rust verifier crate, and *its own other producers*
...
|`affinescript-pixijs` |in-tree bindings |#56 partial: `stdlib/Pixi.affine`
(+ UI/Sound) is the operational binding; 56-A also landed
...
GitHub index #175: 8/8 original sub-issues complete; severity/blocked-by
also in link:TECH-DEBT.adoc[TECH-DEBT.adoc]. Residual WASI work is
#485/#486/#487 (preview1 FS + sockio + wrap-default in tree; native
preview2 re-target still the one-way door); satellite remainder is #56.
...
2026-05-31, status 2026-09-21): #485 — S5 native `wasi:filesystem`
preview1 lowering LANDED (`file_open`/`file_read`/`file_fd_write`/
`file_close`, Effect `File`, gated
`tests/componentize/filesystem_smoke.sh`); #486 — S6c wrap default
flipped to `--command` (S3 reactor smoke now passes `--reactor`);
compiler still emits preview1 core; native-preview2 cleanup remains
the one-way door; #487 — `net_recv`/`net_send`/`net_accept` preview1
lowering LANDED (gated `tests/componentize/sockio_smoke.sh`). WIT
...
|planned (#485 FS slice unblocked 2026-09-21)
|INT-07 |`affinescript-tea` runtime satellite |#182 **CLOSED 2026-07-25** |runtime + run loop
shipped (`TeaApp`/`parseTeaLayout`, Linear-msg enforced); INT-01 dep
cleared (#253). Router/nav = separate INT-09
|INT-08 |DOM reconciler in `affinescript-dom` |#183 **CLOSED 2026-07-25** |reconciler
...
* link:-ELIMINATION.adoc[-ELIMINATION.adoc] — the estate
  -surface elimination ledger (#229), the spine's estate-port arm.
