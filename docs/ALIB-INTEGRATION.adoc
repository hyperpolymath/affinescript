...
(cannot be used after map) - Function `f` must consume its argument - If
`f` panics, remaining elements are dropped safely - No iterator
...
take++_++ownership: Collection++[++A: affine++]++ -++>++ (A,
Collection++[++A++]++)
...
partition++_++consume: Collection++[++A: affine++]++, Function++[++&A
-++>++ Bool++]++ -++>++ (Collection++[++A++]++, Collection++[++A++]++)
...
. *Diversity* - Adds functional {plus} affine-typed language to
portfolio
...
. *Research Value* - Novel combination of affine types {plus} standard
operations
