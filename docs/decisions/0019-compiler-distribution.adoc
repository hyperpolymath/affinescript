...
(2) **Guix/Guix channel.** Aligns with the repo's primary packaging
    (`guix.scm` / `flake.guix`); reproducible, but a narrower audience and no
...
* *Canonical artifact.* `.github/workflows/release.yml`, on a `v*` tag,
  ...
  Guix/Guix and any later npm tail are **additive fetch-derivations over it**,
...
* The install contract is *Releases-first*. Renaming the `SHA256SUMS`
  ...
  version and for Guix/Guix derivations downstream — do not rename without
...
* Browsers and Cloudflare Workers are out of scope for the shim by
  ...
  Runtime Exemptions).
* Reproducible-build consumers are served by the additive Guix/Guix
