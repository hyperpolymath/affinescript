...
* This playbook describes how to **carry an existing codebase across the boundary** — read it if you are translating , TypeScript, or another `-script` family language into AffineScript.
...
| In , `unit` says nothing about whether the function did IO. In AffineScript, the effect row makes it visible.
...
[#portable-http]
=== Portable HTTP ( `fetch` / `Js.Promise` → `Http` stdlib)

 networking is `fetch(...)` returning a `Js.Promise.t`, usually
...
* Every function that transitively calls `Http` carries `/ { Net, Async }`
  ...
  visible in the type, where  hid them in a `Promise`.
...
The following  is a small file buffer that conflates state, lifetime, and IO.
...
This compiles. It is also no better than the  original: the `closed` invariant is still a runtime check, the IO is still invisible, and `close` does not actually consume the buffer.
...
== Co-processor Integerization ( → AffineScript → wasm)
...
patterns for carrying real  across the boundary.
...
A 1:1 port of a  kernel into AffineScript reproduces the  design;
...
[cols="1,2,2"]
|===
| # |  shape | Re-decomposed AffineScript shape
...
`Kernel_IO` does a sandbox check: in  it decodes ASCII bytes to a
...
(guix/guix dev shell + `affinescript` on `PATH`). **New harnesses must call the
...
module-load-time hook in wasm to attach a side effect to. /JS code that
...
| 1.0
| 2026-05-02
| Initial draft.  and TypeScript pattern indices, decision criteria, file-buffer anti-pattern. Companion to `frontier-guide.adoc` v1.0.

| 1.1
| 2026-05-02
| Two additions surfaced by the link:lessons/migrations/idaptik-user-settings.adoc[idaptik UserSettings design walk-through]: (1) service-locator-globals row added to the  pattern index; (2) coupled-writes subsection added to Decision Criteria, with the rule "keep coupling local unless the invariant must be observable across callers." No removals; existing v1.0 guidance unchanged.
