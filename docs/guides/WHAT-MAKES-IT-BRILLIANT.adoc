...
leaks (forgot to close that file?) - Array bounds errors
(`arr++[++999++]++` crashes at runtime) - Division by zero - Race
conditions

*Rust:* Catches all these, but the complexity is overwhelming: -
Lifetime annotations everywhere:
`fn foo++<++'a, 'b: 'a++>++(x: &'a T, y: &'b U)` - Borrow checker fights
you constantly - Steep learning curve

*Haskell/OCaml:* Powerful type systems, but: - No automatic resource
management - Effects are awkward (`IO monad` confusion) - Hard to reason
about performance
...
*Brilliance:* Memory safety {plus} resource safety, but *simpler than
Rust*.
...
*What just happened?* - `Vec++[++0, Int++]++` = Empty vector (type says
it has 0 elements) - `Vec++[++5, String++]++` = Vector with exactly 5
strings - `head()` requires `Vec++[++n{plus}1, T++]++` = At least 1
element - *Compiler proves you never call head() on empty vectors*
...
*What’s `...rest`?* - "`I need a `name: String` field`" - "`I don’t care
what other fields exist`" - True *duck typing* with *compile-time
safety*
...
*Brilliance:* *Flexibility of duck typing* {plus} *safety of static
types*.
...
Start with *link:lessons/01-hello-affinescript.adoc[Lesson 1: Hello
AffineScript]*
...
*Q: When will it be production-ready?* A: Aiming for compiler {plus}
complete tooling in 2026. Follow progress at
https://github.com/hyperpolymath/affinescript[github.com/hyperpolymath/affinescript].

'''''

*Next:* link:lessons/01-hello-affinescript.adoc[Lesson 1: Hello
AffineScript →]
