...
emission. The reference target. WASI preview1 on-demand imports cover
clock/env/args/sockets/filesystem (`file_open`/`file_read`/
`file_fd_write`/`file_close`, `net_recv`/`net_send`/`net_accept`);
`tools/componentize.sh` defaults to the command adapter (#486). Native
preview2 import lowering is still the one-way door.
...
* The satellite ecosystem is mixed maturity — see the satellite registry
  in `ECOSYSTEM.adoc`. `affinescript-tea` (INT-07/#182) and
  `affinescript-dom` (INT-08/#183) shipped runtimes; `affinescript-cadre`
  (INT-09) closed 2026-07-25. `affinescript-pixijs` in-tree bindings
  (`stdlib/Pixi.affine` + UI/Sound) plus 56-A `stdlib/Dom.affine` /
  `stdlib/Console.affine` landed 2026-09-21; full idaptik surface is
  still open (#56). The `-dom-loader` satellite was folded into
  `affine-js` (#489).
