...
This document describes the _face_ architecture (ADR-010) for the AffineScript
...
surface presentation — a different syntax _and_ error vocabulary — layered on
...
| LucidScript (Pu)   | `hyperpolymath/lucidscript`  | `lib/lucid_face.ml`
...
roadmap; the two new faces (ProperScript / TypeScript, RealScript / ) are
...
. Add a preprocessor in a new `lib/<name>_face.ml` (if syntax differs).
. Add branches to each `format_*_for_face` function in `lib/face.ml`.
. Wire the face into `bin/main.ml` (`face_arg`, `parse_with_face`).
