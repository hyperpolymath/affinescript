...
zero consumer churn (genuine -surface residue is #229).
...
==  Block-Module Disposition: One Module Per File (ADR-017)

 `module Name { … }` block modules have no AffineScript block
...
estate →AffineScript ports (#229) carry block modules — a single
...
Decision: *one module per file — split, do not nest.* Each 
...
loader-bridge territory), tracked in `-ELIMINATION.adoc` Tier-4
...
`docs/-ELIMINATION.adoc`; escalation issue #262.

== No Raw/FFI Escape: Typed `extern` Is the Only Host Bridge (ADR-018)

 `%%raw("<host source>")` / `%raw` injects arbitrary untyped host
...
`docs/-ELIMINATION.adoc`; escalation issue #245.
...
over Releases-binaries / Guix-Guix / JSR-shim / combination. The owner
...
single source of truth — Guix/Guix and any npm tail are additive
