...
| RealScript    |               | `real`      | *New — planned*
| CafeScripto   | CoffeeScript          | `cafe`      | Shipped
| LucidScript   | Pu / Haskell  | `lucid`     | Shipped
...
* *RealScript* — , whose ML-family types were _real_ (sound,
...
| RealScript   | `real`, ``       | `realscript`   | `lib/real_face.ml`* · `Face.Real`*      | `hyperpolymath/realscript`*
...
`typescript`/``) once those `Face` variants land. Note: `bin/main.ml`
already carries partial `is_` error-vocabulary plumbing that RealScript
will absorb.
...
* *P1.2* — add `proper`/`real` (+ `typescript`/``) to the CLI `--face`
