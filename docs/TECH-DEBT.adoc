...
upgrade
|STDLIB-04 |Residual `extern` builtins → real implementations.  Triage
+ classification + per-row unblock condition in
link:STDLIB-EXTERN-AUDIT.adoc[STDLIB-EXTERN-AUDIT.adoc] (135 `extern fn`
+ 24 `extern type` across 11 stdlib files, grouped into 4 classes:
built-in / typed-boundary-bridge / adapter-Deno / adapter-Node). |S3
|open — long tail, one row per PR; audit doc landed 2026-05-23
|===
...
structural-conservative recogniser) |S2 |open #234. Composes with
Tranche C/C3 of TYPED-WASM-ROADMAP
