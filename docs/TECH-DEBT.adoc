...
GitHub index #175: **8/8 original sub-issues CLOSED** as of 2026-07-25
(#176 DOC, #177 CORE-01, #178 INT-01, #179 INT-02, #180 INT-03,
#181 INT-04-JSR, #182 INT-07, #183 INT-08). The GitHub ticket is a pure
index whose children are complete; this ledger remains the durable
index. Residual WASI follow-ups #485 (S5 filesystem) and #487
...
Satellite remainder is #56 (56-A Dom/Console + Pixi gap-fill landed;
full idaptik surface still open).
====
...
Stage D  CLOSED ── CORE-01 (#177)  ─┐                            (base lang
         ...
         └─ INT-04 (#181) ◄─┘ (◄ INT-01)                         │
            ADR-014 (#228)  ── estate port unblocker, S1         │
                                                                 ▼
Stage E  ACTIVE  ── typed-wasm convergence hardening
         (#234 closed, #235, #225/#160 line, INT-12) ──► stable multi-producer ABI
----
...
escalations: link:-ELIMINATION.adoc[-ELIMINATION.adoc]
...
Residual (filed 2026-05-31, status 2026-09-21):
#485 S5 native `wasi:filesystem` — *preview1 lowering LANDED*
(`file_open`/`file_read`/`file_fd_write`/`file_close` →
`path_open`/`fd_read`/`fd_write`/`fd_close`, Effect `File`,
dirfd 3, combo canonical-order lock + gated
`tests/componentize/filesystem_smoke.sh` hermetic `--dir`
round-trip). Unblocks INT-06's FS slice.
#486 S6c — *wrap default LANDED* (`tools/componentize.sh` now
defaults to `--command`; S3 reactor smoke requests `--reactor`
explicitly). Compiler still emits preview1 core wasm;
native-preview2 import cleanup remains the one-way door.
#487 socket recv/send/accept — *preview1 lowering LANDED*
(`net_recv`/`net_send`/`net_accept` → `sock_recv`/`sock_send`/
`sock_accept`, Effect `Net`, gated
...
|INT-07 |`affinescript-tea` runtime |S2 |**#182 CLOSED 2026-07-25** —
runtime + run loop shipped (TeaApp/parseTeaLayout, Linear-msg enforced);
INT-01 cleared (#253)
|INT-08 |DOM reconciler |S2 |**#183 CLOSED 2026-07-25** — implemented +
compiles; `.as`→`.affine` fixed; #255 (wasm loop-codegen defect) that
gated the runtime is fixed (PR #257, closed 2026-05-19) — runtime
VERIFIED end-to-end 2026-07-07 via `affinescript-dom/e2e/run.sh`
|INT-09 |`affinescript-cadre` router/navigation runtime |S2 |**CLOSED
2026-07-25** (CadreRouter JS wrapper + E2E tests)
...
|INT-05 |Loader-driven multi-module app bundling |S2 |planned (INT-02 dep
cleared 2026-05-31 via #179; candidate for an issue-spinout)
|INT-06 |Server-side runtime profile (on INT-03 WASI p2) |S2 |planned
(#485 FS slice unblocked 2026-09-21; remaining: native-preview2
clocks/cli/sockets + a real server profile)
|INT-11 |Browser host parity (`affine-js` loader + `affinescript-dom`
reconciler end-to-end) |S2 |**CLOSED 2026-07-25**
|INT-12 |typed-wasm convergence fixtures |S1 |planned (Stage E;
coordinates with `hyperpolymath/typed-wasm` C5.1)
|===
...
|===
|ID |Item |Sev |Status
|SAT-01 |`affinescript-dom` reconciler (skeleton → real) |S2 |**DONE**
(=INT-08, #183 CLOSED 2026-07-25) — runtime VERIFIED e2e 2026-07-07 via
...
|SAT-03 |`affinescript-tea` runtime build-out |S2 |**DONE** (=INT-07,
...
* One PR per ledger unit; `Refs #N` (never `Closes` on the #175 epic — only
  the owner closes it). Residual follow-ups after the original 8 children
  are #485/#486/#487/#56 — do not re-open #175 to track them. Owner may
  close #175 once those residuals are accepted; GitHub write is a PAT
  capability, not a ledger action.
