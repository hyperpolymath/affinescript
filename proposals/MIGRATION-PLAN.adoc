...
. *Variable-string backend* — `String.length`, read-side *indexing*
  ...
  *whitespace trimming* (`trim`, slice 5), and *substring search*
  (`string_find` — flat-safe nested scan, slice 6) now lower to wasm (Phase F,
  see Ledger); `int_to_string`, concat / `++`, and polymorphic `slice` do not
  yet (the last two are gated on a type-access design decision — see Ledger
  F+). Gates every STRING-GATED kernel (e.g. `Kernel_IO`, `DeviceType`, i18n)
  whose work is building/transforming strings rather than scanning them. Lives
...
| F+ | TODO (design decision) | Remaining string ops: (a) *`int_to_string`/`float_to_string`* — `int_to_string` is the last clean autonomous slice (decimal digit loop + sign; handle `INT_MIN` by extracting digits in negative space); `float_to_string` stays host-side under the float-wall. (b) *concat `++` / polymorphic `slice`* — **blocked on an architectural decision**: codegen has *no per-expression type access* (`ExprBinary` is untyped, no type env threaded in), so string `++` (vs the existing list `OpConcat`) and `slice` (array|string) need either typed-AST threading or a dedicated `string_concat` builtin (language-surface call). (c) *`string_get`/`int_to_char`* — deferred (raise on out-of-range; no trap-free wasm analogue). Then the *effect-codegen wall* (module-state / `Date.now` / `Console.log`). See `proposals/EVIDENCE-stringwall-slice6.adoc` §"what remains". *Cluster migration complete; string slices 1-6 landed (read + single-byte write + runtime-length copy + per-byte transform + scan-and-copy + substring search).*
