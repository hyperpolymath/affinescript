...
. *Variable-string backend* — `String.length`, read-side *indexing*
  ...
  and *whitespace trimming* (`trim` — scan-and-copy, slice 5) now lower to wasm
  (Phase F, see Ledger); `string_find`, `int_to_string`, concat / `++`, and
  polymorphic `slice` do not yet (the last two are gated on a type-access
  design decision — see Ledger F+). Gates every STRING-GATED kernel (e.g.
