...
Dogfooding the idaptik  -> AffineScript -> wasm migration surfaced
...
. *Wrong toolchain.* They still shell out to a **retired** podman image,
  ...
  favour of host-native `affinescript` on PATH (guix/guix dev shell). A
...
The build network is *github-only*. `crates.io`, ``,
...
* *Network:* `crates.io` / `` / `opam.ocaml.org` / launchpad ->
...
* *Migration corpus:* idaptik 107,758 LOC  / 21,540 LOC
