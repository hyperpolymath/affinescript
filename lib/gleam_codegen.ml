...
and gen_stmt = function
  | StmtLet { sl_pat = PatVar id; sl_value; sl_ty; _ } ->
      Printf.sprintf "let %s = %s" (mangle id.name)
        (gen_value_with_hint sl_ty sl_value)
  | StmtLet _ -> ""
  | StmtExpr e -> gen_expr e
  | _ -> ""

(* Gleam record literals require the constructor name: [Point(x: 40, y: 2)],
   not bare [(x: 40, y: 2)]. When the surrounding let supplies the type
   annotation, lift the type name onto the record literal. *)
and gen_value_with_hint (ty_hint : type_expr option) (e : expr) : string =
  match e, ty_hint with
  | ExprRecord { er_fields; _ }, Some (TyCon id) ->
      let fs = List.map (fun (n, e_opt) ->
        let v = match e_opt with Some e -> gen_expr e | None -> mangle n.name in
        Printf.sprintf "%s: %s" (mangle n.name) v
      ) er_fields in
      Printf.sprintf "%s(%s)" id.name (String.concat ", " fs)
  | _ -> gen_expr e
...
let prelude = {|// AffineScript Gleam runtime (MVP)
import gleam/io

pub fn print(s: String) -> Nil { io.print(s) }
pub fn println(s: String) -> Nil { io.println(s) }
