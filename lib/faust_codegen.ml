...
(* Faust accepts the shared math-builtin set verbatim; no Faust-specific
   intrinsics beyond the stdlib intersection. *)
let faust_builtins = math_builtins
...
(* Faust's canonical entry name is [process]; fall back to main / kernel /
   first-fn via Kernel_sublang's shared finder. *)
let faust_pick_entry (program : program) : fn_decl =
  pick_entry ~names:["process"; "main"; "kernel"] program
...
let generate (program : program) (_symbols : Symbol.t) : string =
  ...
  let entry = faust_pick_entry program in
...
let codegen_faust (program : program) (symbols : Symbol.t) : (string, string) result =
  ...
  with
  | Unsupported msg -> Error ("Faust backend: " ^ msg)
