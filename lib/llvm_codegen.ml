...
(* Target triple for the emitted module. Defaults to the x86_64 host; set
   [AFFINESCRIPT_LLVM_TRIPLE] to cross-target — e.g.
   [aarch64-linux-android] for the native Android backend, [aarch64-apple-ios]
   for iOS, or any LLVM triple. The emitted IR is target-independent (verified:
   the same .ll lowers to both x86-64 and aarch64 objects), so only the triple
   string changes. The env var is the low-level knob (mirroring
   [AFFINESCRIPT_SHOW_MENHIR_NOISE] / [AGDA_STDLIB]); a [compile --target] CLI
   flag is the planned ergonomic front-end. *)
let llvm_target_triple () : string =
